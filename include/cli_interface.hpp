...
consteval bool has_help_option() {
  return apply_to_options([](auto... opts) { return ((opts.name == "help") || ...); });
}
...
inline options parse(int argc, char* argv[], error_code& ec) noexcept {
  ...
  if (!has_help_option() && argc > 1 && std::string_view(argv[1]) == "--help") {
#ifdef get_help_text
...
inline options parse_or_exit(int argc, char* argv[]) {
  ...
  if (ec) {
    std::cerr << "an error occurred: " << to_string(ec.what);
    std::exit(1);
  }
  return o;
}
